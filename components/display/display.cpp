...
void oled_init(void)
{
    ...
    ESP_LOGI(TAG, "XiaoZhi SSD1306 128x64 siap: SDA=%d SCL=%d ADDR=0x%02X SPEED=400kHz MIRROR=XY", OLED_SDA_PIN, OLED_SCL_PIN, OLED_I2C_ADDR);
}
...
static void draw_open_eye(int cx, int cy, int look)
{
    const int r = 14;
    int px = cx, py = cy;
    if (look == 1) px -= 5;
...
void display_render_mochi(int expr, int step, int sX, int sY, int arahLirik)
{
    ...
    /* expr 2 + step 2 = HAPPY: mata tersenyum/tertutup melengkung */
    if (expr == 2 && step == 2) {
        draw_happy_eye(L, Y);
        draw_happy_eye(R, Y);
    }
    /* expr 2 = SPEAKING/HAPPY normal; step 1 = blink */
    else if (expr == 2) {
        if (step == 1) {
            draw_blink_eye(L, Y);
            draw_blink_eye(R, Y);
        } else {
            draw_open_eye(L, Y, arahLirik);
            draw_open_eye(R, Y, arahLirik);
        }
    }
    /* expr 6 = SAD: mata menutup */
    else if (expr == 6) {
        draw_sad_eye(L, Y);
        draw_sad_eye(R, Y);
    }
...
void face_render(void)
{
    ...
    display_render_mochi(expr, step, 0, 0, 0);
}
