    ...
    while (1) {
        ...
        if (!assistant_active) {
            ...
            if (now_debug_us - last_debug_us >= 1000000) {
                ...
                ESP_LOGI("WAKE_DEBUG",
                         ...
                         (unsigned)buffer_pos, (long)max_abs, detect_calls,
                         last_wake_result, wake_chunk_samples, (unsigned)bytes_read);
            }
        ...
        if (buffer_pos >= 3200) {
            ...
            if (!audio_turn_active) {
                if (has_activity) websocket_send_audio_data(audio_buffer, 3200);
                else {
                    silent_frames++;
